...
- `lib/llm.ts` — the LLM layer, provider-isolated. Gemini via raw fetch, model fallback chain `gemini-2.5-flash` → `gemini-2.5-flash-lite` on 429/503 with a second sweep after backoff. System prompt carries Miles's persona, Telegram HTML formatting rules, and pre-computed event times (never let the model derive weekdays).
